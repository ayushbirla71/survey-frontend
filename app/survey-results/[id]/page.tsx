...
import { ArrowLeft, Download, Share2, Users, Clock, CheckCircle, RefreshCw } from "lucide-react"
...
export default function SurveyResults() {
  ...
  const {
    ...
    return apiWithFallback(() => surveyResultsApi.getSurveyResults(surveyId), demoResult)
  }, [surveyId])
  ...
  } = usePaginatedApi((params) => surveyResultsApi.getIndividualResponses(surveyId, params), { page: 1, limit: 10 })

  // Use API data if available, otherwise use demo data
  const survey = results || surveyResults[surveyId] || surveyResults["survey-1"]
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={handleRefresh} disabled={resultsLoading}>
        ...
        {resultsError && (
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-800 text-sm">⚠️ Using demo data - API connection failed: {resultsError}</p>
          </div>
        )}
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          <Card>
            <CardContent className="flex items-center p-4">
              ...
              <div>
                <p className="text-2xl font-bold">{resultsLoading ? "..." : survey.stats?.totalResponses || 0}</p>
          ...
          <Card>
            <CardContent className="flex items-center p-4">
              ...
              <div>
                <p className="text-2xl font-bold">{resultsLoading ? "..." : survey.stats?.completionRate || 0}%</p>
          ...
          <Card>
            <CardContent className="flex items-center p-4">
              ...
              <div>
                <p className="text-2xl font-bold">{resultsLoading ? "..." : survey.stats?.avgTime || 0} min</p>
                ...
                <CardContent>
                  ...
                  {question.type === "rating" && (
                    <div className="space-y-4">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-violet-600">{question.averageRating}</div>
                  ...
                  {question.type === "text" && (
                    <div className="space-y-2">
                      <p className="text-sm text-slate-500">Recent responses:</p>
          ...
          <Card>
            ...
            <CardContent>
              {responsesLoading ? (
                ...
                <div className="space-y-4">
                  {(individualResponses || survey.individualResponses || []).map((response, index) => (
                    <div key={response.id || index} className="rounded-lg border p-4">
                      <div className="mb-2 flex items-center justify-between">
                        <span className="font-medium">Response #{response.id}</span>
                        <div className="flex items-center gap-2">
                          <Badge variant="outline">{response.completionTime} min</Badge>
                          <span className="text-sm text-slate-500">{response.submittedAt}</span>
                        </div>
                      </div>
                      <div className="space-y-2">
                        {response.answers?.map((answer, i) => (
                          <div key={i} className="text-sm">
                            ...
                            <span className="ml-2 text-slate-600">{answer.answer}</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="mt-6 flex items-center justify-between">
                  ...
                  <Button
                    ...
                    disabled={pagination.page === pagination.totalPages || responsesLoading}
                  >
                    Next
                  </Button>
                </div>
        ...
        <TabsContent value="export">
          <Card>
            <CardHeader>
              ...
              <p className="text-slate-500">Download your survey results in various formats</p>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
